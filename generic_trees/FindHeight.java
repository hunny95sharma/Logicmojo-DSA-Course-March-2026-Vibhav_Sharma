...
public class FindHeight {
	...
	public static int fetchHeightOfTree(TreeNode<Integer>root) {
		if(root==null)
			return 0;
		int childCount=root.children.size();
		int ans=0;
		for(int i=0;i<childCount;i++) {
			int temp=fetchHeightOfTree(root.children.get(i));
			if(temp>ans)
				ans=temp;
		}
		return ans+1;
	...
	public static void main(String[] args) {
		...
		printLevelWise(root);
		System.out.println("Node count greater than x "+fetchHeightOfTree(root));
	}
