...
public class NextLargerElement {
	...
	public static TreeNode<Integer> findNextLargerNode(TreeNode<Integer> root, int n){
		...
		if(root.data>n) {
			res=new TreeNode<Integer>(root.data);
			ans=root.data;
		}
		for(int i=0;i<childCount;i++) {
			TreeNode<Integer>childNode=root.children.get(i);
			TreeNode<Integer>tempNode=findNextLargerNode(childNode,n);
			if(tempNode!=null&&tempNode.data<ans) {
				ans=tempNode.data;
				res=tempNode;
			}
		}
	...
	public static void main(String[] args) {
		...
		printLevelWise(root);
		System.out.println(findNextLargerNode(root,9).data);
	}
